...
import { POSITION_TO_CAPABILITY } from './positionCapability';
...
(runDb ? describe : describe.skip)('Position -> Capability mapping (integration)', () => {
  beforeAll(async () => {
  ...
  it('all capability codes referenced by POSITION_TO_CAPABILITY exist in DB', async () => {
    const codes = Array.from(new Set(Object.values(POSITION_TO_CAPABILITY)));
    for (const code of codes) {
      const cap = await prisma.capability.findUnique({ where: { code } });
      expect(cap, `Capability code missing: ${code}`).toBeTruthy();
    }
  });

  it('GET /segments/:id/positions uses centralized mapping for requiredCapabilityCode', async () => {
    ...
    const items: Array<{ name: string; requiredCapabilityCode: string | null }> = res.body;
    // find ledscherm regie -> must be SCHERM_REGISSEUR
    const led = items.find((i) => i.name === 'ledscherm regie');
    expect(led).toBeTruthy();
    expect(led!.requiredCapabilityCode).toBe('SCHERM_REGISSEUR');
  });

  it('POST /segments/:id/assignments enforces required capability from centralized mapping', async () => {
    ...
    // Create a person without SCHERM_REGISSEUR capability
    const p = await prisma.person.create({ data: { name: 'Alice', gender: 'female' } });

    // Assign crew at production level with some other capability so they are considered crew
    const someCap = await prisma.capability.findFirst({ where: { code: 'REGISSEUR' } });
    const ledCap = await prisma.capability.findFirst({ where: { code: 'SCHERM_REGISSEUR' } });
    expect(someCap && ledCap).toBeTruthy();
    await prisma.personCapability.create({ data: { personId: p.id, capabilityId: someCap!.id } });
    await prisma.matchRoleAssignment.create({ data: { matchScheduleId: match.id, personId: p.id, capabilityId: someCap!.id } });

    // Try to assign to ledscherm regie (should fail - lacks required cap)
    const bad = await request(app).post(`/api/production/segments/${seg.id}/assignments`).send({ personId: p.id, positionId: posLed.id });
    expect(bad.status).toBe(422);

    // Now give the required capability and try again
    await prisma.personCapability.create({ data: { personId: p.id, capabilityId: ledCap!.id } });
