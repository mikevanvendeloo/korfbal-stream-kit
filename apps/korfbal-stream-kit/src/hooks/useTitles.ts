...
export function useVmixTitles(productionId: number) {
  return useQuery({
    ...
    queryFn: async (): Promise<VmixTitleItem[]> => {
      const res = await fetch(url(`/api/vmix/production/${productionId}/titles`));
...
export function useProductionTitles(productionId: number) {
  return useQuery({
    ...
    queryFn: async (): Promise<TitleDefinition[]> => {
      const res = await fetch(url(`/api/production/${productionId}/titles`));
      if (!res.ok) throw new Error(await extractError(res));
      return res.json();
    },
  });
...
export function useUpdateProductionTitle(productionId: number) {
  ...
  return useMutation({
    mutationFn: async (input: { id: number; name?: string; enabled?: boolean; parts?: TitlePart[] }): Promise<TitleDefinition> => {
      ...
      const res = await fetch(url(`/api/production/${productionId}/titles/${input.id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(sanitized),
      });
...
export function useDeleteProductionTitle(productionId: number) {
  ...
  return useMutation({
    mutationFn: async (titleId: number) => {
      const res = await fetch(url(`/api/production/${productionId}/titles/${titleId}`), { method: 'DELETE' });
...
export function useReorderProductionTitles(productionId: number) {
  ...
  return useMutation({
    mutationFn: async (ids: number[]) => {
      const res = await fetch(url(`/api/production/${productionId}/titles:reorder`), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
...
export function useApplyDefaultTitles(productionId: number) {
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await fetch(url(`/api/admin/vmix/production/${productionId}/titles/use-default`), { method: 'POST' });
      if (!res.ok && res.status !== 204) throw new Error(await extractError(res));
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['production', productionId, 'titles'] });
...
export function useTitleTemplates() {
  return useQuery({
    ...
    queryFn: async (): Promise<TitleDefinition[]> => {
      const res = await fetch(url(`/api/admin/vmix/title-templates`));
      if (!res.ok) throw new Error(await extractError(res));
      return res.json();
    },
  });
...
export function useUpdateTitleTemplate() {
  ...
  return useMutation({
    mutationFn: async (input: { id: number; name?: string; enabled?: boolean; parts?: TitlePart[] }): Promise<TitleDefinition> => {
      const sanitized = input.parts ? { ...input, parts: input.parts.map(sanitizePart) } : input;
      const res = await fetch(url(`/api/admin/vmix/title-templates/${input.id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(sanitized),
      });
...
export function useDeleteTitleTemplate() {
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(url(`/api/admin/vmix/title-templates/${id}`), { method: 'DELETE' });
      if (!res.ok && res.status !== 204) throw new Error(await extractError(res));
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin', 'vmix', 'title-templates'] }),
  });
...
export function useReorderTitleTemplates() {
  ...
  return useMutation({
    mutationFn: async (ids: number[]) => {
      const res = await fetch(url(`/api/admin/vmix/title-templates:reorder`), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids }),
      });
